...
* **🌌 同名・類似名の海外都市との誤判定を防ぐ国コード（JP）優先処理の導入 (v1.7.6) [NEW]**
  - **位置検索の国別優先順位設定**: 「岩手（Iwate）」のように、英語表記した際に海外の別地域の地名（パプアニューギニアの Iwatebu など）と前方一致・類似判定されてリストの最上位になってしまう現象が発生していました。これを防ぐため、位置検索結果から最大10件の候補を取得し、その中に国コード `JP`（日本）の候補が存在する場合は自動的にそれを最優先で選択するロジックを実装。これにより、日本の地名での検索精度が大幅に向上しました。

* **🌌 日本語都市名クエリでの天気位置情報ミスマッチの解消 (v1.7.5)**
  - **英字変換パラメーター指示の追加**: Open-Meteoの地名検索（Geocoding API）は日本語の都市名（「沖縄」など）を直接受け付けると位置情報を正しく検出できず、結果として東京の座標にフォールバックしてしまっていました。今回の改修で、LLMに対する天気予報APIの引数説明（スキーマ定義）に「都市名を必ず英語表記（Okinawaなど）に翻訳して呼び出すこと」という明確なプロンプト指示を追加。これにより、モデルが自動的に正しい英名で地名検索を呼び出し、正確な座標から天気を取得できるようになりました。
  - **ローカルフォールバック定義の強化**: もしAPI連携で位置情報が引けなかった場合の保険として、主要な日本の都道府県・大都市（「沖縄」「横浜」「名古屋」「神戸」「広島」等）を日本語・英語の双方で座標マップに事前定義しました。
