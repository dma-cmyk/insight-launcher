...
* **🌌 ドラッグ並び替えのチャタリングバグ修正 (v1.5.9) [NEW]**
  - **ドラッグ並び替え時のデバウンス制御の導入**: お気に入りアプリアイコンをドラッグして並び替える際、1フレーム単位で高速に並び替え判定が繰り返されることで関係のないアプリまで意図せず動いてしまう「チャタリングバグ」を完全に解消。150ms のクールダウン（デバウンス）処理を導入し、レイアウトの再描画が完了するまで次の並び替えを安全に抑止することで、指の動きに吸い付くような超安定した並び替えを実現しました。
