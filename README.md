...
* **🌌 お気に入りドラッグ位置ズレ＆瞬間移動バグの完全解消 (v1.6.0) [NEW]**
  - **ドラッグ中オフセットの逆変換による物理座標補正**: お気に入りのドラッグ中にアイコンが瞬間移動したり、他のお気に入りまで崩壊するバグの根本原因であった「再レイアウト時の座標追跡ズレ」を解決。ドラッグ対象の Composable は `.offset` 修飾子で指の位置へ移動しているため、`onGloballyPositioned` から取得する絶対座標にオフセットが蓄積されてしまい、次の判定で指の真下から計算上の中心が瞬間的に吹き飛ぶ現象が起きていました。
  - **基準座標アライメント (Baseline Coordinates) 設計**: `onGloballyPositioned` 側で、ドラッグ中アイテムの場合に `bounds.translate(-dragOffset)` を適用し、オフセットを相殺した「本来のグリッド基準座標」を `itemPositions` に保存するように修正。これにより、どんなに高速にドラッグしても完全に指の直下にアイテムが追従し、意図した通りの極上並び替え体験を実現しました。
