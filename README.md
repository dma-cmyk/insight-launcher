...
* **🌌 お気に入りドラッグフリーズバグの解決＆座標追跡の極限安定化 (v1.6.1) [NEW]**
  - **ドラッグアイテムの座標更新スキップ設計**: `v1.6.0` のオフセット逆変換において、並び替え発生時の Compose 内部インデックス更新と `onGloballyPositioned` の実行ラグによって、逆計算が狂い座標データが破損してドラッグがロック（フリーズ）してしまう現象を解消。
  - **静的アイテムのみによるセル座標維持**: ドラッグ中のアイテム自身の `onGloballyPositioned` による位置記録を完全にスキップ（`!isCurrentDragging` 時のみ書き込み）するように変更。グリッド各セルの「本来の基準座標」は、他のドラッグしていない静的アイテムが測定した座標のみで完璧に維持されるため、複雑な逆オフセット計算が不要になり、どれだけ素早くドラッグ＆ドロップを繰り返しても絶対にロックしない極めて堅牢な安定動作を確立しました。
