...
* **🏠 デフォルトのホームアプリ設定機能 [NEW]**
  * 設定画面からワンタップでAndroidシステム設定の「デフォルトのホームアプリ」選択画面へ遷移し、本アプリをスマートフォンの主画面として簡単に設定できるようになりました。
* **🎨 プレミアムなオリジナルアイコンへの刷新**
...
* **⚙️ アプリシステム設定へのダイレクトアクセス**
  ...
  * また、詳細ダイアログ内の起動ボタンと設定ボタンの幅・高さが美しく統一され、視認性が向上しました [NEW]。
* **🌐 ローカライズと言語切り替えの最適化**
