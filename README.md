...
* **🌌 お気に入り長押し競合バグの完全解消＆AI検索結果のマークダウンレンダリング対応 (v1.5.6) [NEW]**
  ...
  - **AI詳細解説のマークダウンレンダリング対応 (`MarkdownText`)**: AIアシスタント画面で取得・解析した GitHub リポジトリや F-Droid アプリの詳細解説（README要約など）を、プレーンテキスト表示からカスタムのマークダウンパーサー (`MarkdownText`) を用いたリッチレンダリング表示へと進化させました。見出し（`#`, `##`, `###`）や箇条書きリスト（`-`, `*`, `•`）の自動インデント、および `**太字**` のボールド表示に対応し、視認性を劇的に向上させました。
