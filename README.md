...
* **🌌 並び替え挙動を「スワップ（2者個別入れ替え）」方式に差し戻し (v1.6.3) [NEW]**
  - **個別入れ替え (Swap) への再最適化**: お気に入りのドラッグ時に、通過パスにある隣接アプリがすべて押し出されて動いてしまう（関係のないアプリまで位置がズレる）不満を解消するため、並び替えのアルゴリズムを元の「ドラッグ中アプリと重ねた先アプリの2つだけを入れ替える（Swap）」方式に完全に差し戻しました。
  - **化け防止と極上ドラッグの両立**: 以前のスワップで発生していた「アイコンが化ける・重複する」バグは、これまでに導入した `key` アライメントと `onGloballyPositioned` のドラッグスキップ設計によって完全に解決されています。これにより、他のアプリに一切影響を与えずに、狙ったアプリ同士だけを直感的に入れ替えられるようになりました。
