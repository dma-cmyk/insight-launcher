...
* **🔍 セマンティックアプリ検索 (AI意図解析) & 類似度スコア表示**
  * 「カレンダー」のようなアプリ名での検索はもちろん、「予定を管理したい」「ネットサーフィンしたい」といった自然言語 of 意図を入力して、最適なアプリをAIが探し出します。
...
1. **プロジェクトのインポート**
   Android Studio を開き、本プロジェクト of ルートディレクトリをインポートします。
...
3. **SDKパスの確認 (`local.properties`)**
   `local.properties` を作成または編集し、お使いの環境 of Android SDK のパスを指定してください。
   ```properties
...
2. 最新リリース of `Assets` から `app-debug.apk` をダウンロード
