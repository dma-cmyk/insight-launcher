...
* **🌌 AIアシスタントによる使用履歴・お気に入りアプリの参照機能 (v1.7.10) [NEW]**
  - **get_app_usage_stats MCPツールの追加**: AIアシスタントが「最近使ったアプリ」「よく使うアプリ」「お気に入りに追加したアプリ」のリストを取得できる専用のMCPツールを搭載しました。これにより、AIに「お気に入りのアプリから〇〇を探して」や「最近使ったアプリを起動して」といった、使用状況に基づいたよりパーソナライズされたアシストが可能になりました。

* **🌌 AIアシスタント起動アプリの履歴反映 (v1.7.8)**
  - **AI起動時のアプリ使用履歴反映**: AIアシスタント経由でアプリを起動（MCPの `launch_installed_app` ツール実行）した際にも、その起動履歴が記録されるよう修正しました。これにより、AIから開いたアプリもホーム画面の「最近使ったアプリ」や「使用頻度順」に正しく反映されるようになります。

* **🌌 AI解析カテゴリ・概要生成時の厳格な言語制約対応 (v1.7.7)**
  - **カスタム指示・言語指定の確実な反映**: アプリの一括解析（バルク）時や、最終的なカテゴリの集約（マージ）時において、AIが設定画面で指定された言語（日本語など）を無視して英語で出力してしまう現象を解消。プロンプトに「指定言語が英語以外の時は、固有名詞を除き絶対に英語で出力してはならない」という強制ルール（CRITICAL LANGUAGE RULE）を追加し、カスタム指示と選択言語設定が確実に適用されるように修正しました。

* **🌌 同名・類似名の海外都市との誤判定を防ぐ国コード（JP）優先処理の導入 (v1.7.6)**
  - **位置検索の国別優先順位設定**: 特定の地域（例：一部の地方名）を英語表記した際に、海外の別地域の類似する地名と前方一致・類似判定されてリストの最上位になってしまう現象が発生していました。これを防ぐため、位置検索結果から最大10件の候補を取得し、その中に国コード `JP`（日本）の候補が存在する場合は自動的にそれを最優先で選択するロジックを実装。これにより、日本の地名での検索精度が大幅に向上しました。

* **🌌 日本語都市名クエリでの天気位置情報ミスマッチの解消 (v1.7.5)**
  - **英字変換パラメーター指示の追加**: Open-Meteoの地名検索（Geocoding API）は日本語の都市名を直接受け付けると位置情報を正しく検出できず、結果としてデフォルトの座標にフォールバックしてしまっていました。今回の改修で、LLMに対する天気予報APIの引数説明（スキーマ定義）に「都市名を必ず英語表記（ローマ字）に翻訳して呼び出すこと」という明確なプロンプト指示を追加。これにより、モデルが自動的に正しい英名で地名検索を呼び出し、正確な座標から天気を取得できるようになりました。
  - **ローカルフォールバック定義の強化**: もしAPI連携で位置情報が引けなかった場合の保険として、主要な国内大都市・地方自治体の座標マップを日本語・英語の双方で事前定義しました。

* **🌌 Open-Meteo Geocoding API 連携による全世界都市の天気検索対応 (v1.7.4)**
  - **動的座標取得の追加**: 天気予報ツール（`get_weather_info`）において、従来は一部の主要都市しか座標が固定されておらず、その他の都市はハッシュ計算による不正確な座標で取得されていました。今回のアップデートで無料の Geocoding API を統合し、ユーザーが指定した任意の地名（例：国内外の主要都市や地域）の正確な緯度経度をリアルタイムで取得した上で Open-Meteo から正確な気象情報を取得できるようになりました。
...
* **🌌 フォールバック時デバッグ情報の画面表示対応 (v1.7.2)**
  - **デバッグ情報の出力**: MCPツールのリクエストが失敗し、ツールなしのフォールバックリトライが発生した際、最初の失敗の原因（Google APIのエラーコード・レスポンス内容）を画面の回答テキスト下部に自動で付与して表示するようにしました。これにより、ADBログを取得しなくてもツール呼び出しエラーの原因を画面上で特定しやすくなります。

* **🌌 複数パート構成レスポンス（会話＋ツール実行）の解析対応 (v1.7.1)**
  - **ツール呼び出しの無視バグ解消**: モデルがツール実行（関数呼び出し）の前に「〜をお調べします」といった前置きテキストを同時に返してきた場合、最初のパート（テキストのみ）だけを見てツール実行指示を見失いループを終了していたバグを修正。返却されたすべてのパート（`parts`）を検証し、関数呼び出しが含まれている場合は漏れなくMCPツールをキックするように変更しました。

* **🌌 Gemini API 400エラーの完全解決＆MCPツール連携の完全対応 (v1.7.0)**
  - **引数なしツールのパラメータ省略**: `get_current_time_and_date` や `get_device_status` などの引数不要のMCPツールにおいて、空の `properties: {}` を送信するとGemini APIが `400 Bad Request (INVALID_ARGUMENT)` を返してしまっていたため、パラメータスキーマを完全に省略（`null`）して送信するように改修しました。
  - **スキーマ型名の完全小文字化**: API仕様に厳密に準拠するため、すべてのJSON Schemaおよびツールパラメータ内のデータ型名（`OBJECT` / `STRING` など）を自動で小文字（`object` / `string`）に再帰変換して送信する正規化処理を追加しました。

* **🌌 400エラー時の自動フォールバック＆ログ詳細表示 (v1.6.9)**
  - **フォールバックリトライ**: MCPツール呼び出し時にエラー（400など）を検知した場合、アシスタント機能全体のフリーズを防ぐため、ツールなしで自動的に回答をリビルドするフォールバック処理を実装。
  - **エラー詳細の可視化**: 通信エラー発生時にGoogleが返した詳細なエラー内容を解析して画面上およびログに出力できるようにしました。

* **🌌 AIアシスタントのMCPツール呼び出しフローの修正 (v1.6.8)**
  - **ロール名の正規化**: ツール応答のロールを Gemini REST API v1beta 仕様に準拠した `role = "function"` に修正。
  - **ツール＋構造化出力の競合解消**: `tools` と `responseMimeType`/`responseSchema` を同時にリクエストに含めると発生していた 400 エラーを解消するため、ツール呼び出しフェーズと構造化出力フェーズを分離。

* **🌌 AIアシスタント内の組み込みMCP（天気・日付等）呼び出し時の400エラーの修正 (v1.6.6)**
  - **ロール名エラーの解消**: Gemini APIへのツール応答（Function Response）送信時、不正なロール名 `role = "function"` を送信していたバグを修正。正しい `role = "user"` に修正し、天気や日付などのビルトインMCPツールの利用時に発生していた 400 Bad Request エラーを解消しました。

* **🌌 AIアシスタントの F-Droid & GitHub リアルタイム検索の全表示＆ページネーション（追加読み込み）対応 (v1.6.5)**
  - **検索結果の全表示化**: 従来の上位制限（F-Droid 10件、GitHub 5件）を撤廃し、1ページ内に取得されたすべての検索結果の表示に対応しました。
  - **アプリ内「更に検索する」ボタンの追加**: 検索結果リストの最下部に「更に検索する (次のページ)」ボタンを追加。ブラウザへ遷移することなく、その場で次のページデータを取得してリスト末尾にシームレスに追記マージします。
  - **追加ロード時の自動翻訳追従**: 追加で取得したリポジトリやパッケージの説明文についても、ユーザーが設定した表示言語（日本語など）へとGeminiが自動翻訳して表示するローカライズ追従が動作します。

* **🌌 コンパクト表示時のドラッグ並び替えアイコンすり替わり（化け）バグの解消 (v1.6.4)**
  - **ループ内 key(app.packageName) の完全導入**: コンパクト表示の並び替えグリッド（`CompactDraggableFavoritesGrid` および `CompactDraggableFavoritesDashboardGrid`）において、ループ要素である `Box` に `key` が指定されていなかったために、並び替え発生時に Composable がインデックスベースで再利用され、指の下のアイコン画像が重ねた相手のアイコンと入れ替わって（化けて）しまう重大な表示バグを解決。
  - **ユニークな追跡による安定化**: `key` の追加により、ドラッグ中も各アプリの Composable が正しく追跡され、画像の化けや不自然な切り替わりが一切起きずに、指の真下のアプリと重なったアプリだけが美しくスワップされる完璧な挙動が完成しました。

* **🌌 並び替え挙動を「スワップ（2者個別入れ替え）」方式に差し戻し (v1.6.3)**

* **🌌 コンパクト表示時のお気に入り長押しドラッグ開始バグの解消 (v1.6.2)**

* **🌌 お気に入りドラッグフリーズバグの解決＆座標追跡の極限安定化 (v1.6.1)**

* **🌌 お気に入りドラッグ位置ズレ＆瞬間移動バグの完全解消 (v1.6.0)**

* **🌌 ドラッグ並び替えのチャタリングバグ修正 (v1.5.9)**

* **🌌 お気に入りスライド並び替え時の要素シフト＆アプリ詳細スワイプ切り替えの極上 HorizontalPager 対応 (v1.5.8)**
