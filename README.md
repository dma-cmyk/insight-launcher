...
* **🌌 AIアシスタントのMCPツール呼び出しフローの完全修正 (v1.6.8) [NEW]**
  - **ロール名の正規化**: ツール応答のロールを Gemini REST API v1beta 仕様に準拠した `role = "function"` に修正（前バージョンでの誤修正を修正）。
  - **ツール＋構造化出力の競合解消**: `tools` と `responseMimeType`/`responseSchema` を同時にリクエストに含めると発生していた 400 Bad Request (INVALID_ARGUMENT) エラーを解消。ツール呼び出しフェーズでは構造化出力設定を省略し、ツール処理完了後に構造化JSON出力のみの最終リクエストを送信する2段階方式に改修しました。

* **🌌 引数なしMCPツール（天気・時間等）呼び出し時のスキーマ 400 エラーの完全解消 (v1.6.7)**
  - **パラメータスキーマの修正**: `get_current_time_and_date` や `get_device_status` のような引数を持たない（空パラメータの）ビルトインMCPツールのスキーマが `null` としてGemini APIに送信されていたバグを修正。引数がない場合でも空のオブジェクト型スキーマを正しく送信するように修正し、依然として発生していた `400 Bad Request` (INVALID_ARGUMENT) エラーを完全に解消しました。
