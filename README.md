...
* **🌌 コンパクト表示時のお気に入り長押しドラッグ開始バグの解消 (v1.6.2) [NEW]**
  - **長押しイベントの親伝播の確保（combinedClickable 回避）**: コンパクト表示のアイテム（`CompactAppItemWithSummary`）において、ドラッグ並び替え時（`onLongClick = null`）に長押しイベントが子要素に消費され、親の長押しドラッグ検出（`detectDragGesturesAfterLongPress`）が反応しなくなるバグを解決。
  - **動的クリック修飾子の導入**: `onLongClick` が `null` の場合は通常の `.clickable` を、`null` でない場合のみ `.combinedClickable` を適用するように動的な Modifier 分岐を実装。これにより、長押しが100%確実に親のドラッグ処理に届き、コンパクト表示でも長押しから即座に並び替えドラッグが開始できるようになりました。
